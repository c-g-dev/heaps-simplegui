...
private class Fake extends Object {
	...
	override function getBoundsRec(relativeTo:Object, out:h2d.col.Bounds, forSize:Bool) {
		...
		if (dd.selectedItem >= 0) {
			var item = @:privateAccess dd.getItemForLabeling(dd.selectedItem);
...
class Dropdown extends Flow {
	...
	**/
				...
				if (clickPos.y >= bds.yMin && clickPos.y < bds.yMax) {
					if (highlightedItem != i) {
						...
						if (cursor.tile.width != 0 && cursor.tile.height != 0) {
							...
							cursor.x = 1;
							cursor.y = item.y;
							@:privateAccess cursor.tile.width = minWidth - 2;
							@:privateAccess cursor.tile.height = Std.int(item.getSize().height);
						}
						onOverItem(item);
					}
					break;
    ...
    public dynamic function getItemForLabeling(index) {
		return getItems()[index];
	}
